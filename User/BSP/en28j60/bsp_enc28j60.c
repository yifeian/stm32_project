...
uint8_t SPI_NETWORK_Init(void)
{
  ...
  GPIO_Init(NETWORK_ENC28J60_RST, &GPIO_InitStructure);
	...
	/*!< 配置 SPI_NETWORK_SPI 引脚: INT */
  GPIO_InitStructure.GPIO_Pin = NETWORK_INT_PIN;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
  GPIO_Init(NETWORK_ENC28J60_INT, &GPIO_InitStructure);
  ...
  enc28j60_dev.macaddr[3]=(addr>>16)&0XFF;	//低三字节用STM32的唯一ID
  enc28j60_dev.macaddr[4]=(addr>>8)&0XFFF;
  enc28j60_dev.macaddr[5]=addr&0XFF;
 ...
 {
	 ENC28J60_CS(0);
	 SPI_NETWORK_SendByte(ENC28J60_READ_BUF_MEM);
	 while(len)
	 {
		 len--;
		 *data=(u8)SPI_NETWORK_SendByte(0);
		 data++; 
	 }
	 *data='\0';
	 ENC28J60_CS(1);
 ...
 {
	 ENC28J60_CS(0);
	 SPI_NETWORK_SendByte(ENC28J60_WRITE_BUF_MEM);
	 while(len)
	 {
		 len--;
		 SPI_NETWORK_SendByte(*data);
...
u8 SPI_NETWORK_ReadByte(void)
{
  ...
  return (SPI_NETWORK_SendByte(0xff));
}
